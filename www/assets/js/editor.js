...
var onLoad = function() {
	...
	var editor = CodeMirror.fromTextArea($editorField[0], editorOpts);
	
	var $body = $('body');
	var appRootUrl = $body.data('rootUrl');
	
	// TODO Load mode dynamically
	...
	for (var index in modeData)
	{
		var mode = modeData[index];
		console.log(mode);
		//if (mode.mode != 'null') $body.append('<script src="' + appRootUrl + 'assets/js/vendor/codemirror/mode/' + mode.mode + '/' + mode.mode + '.js"></script>');
		//var $entry = $('<li><a data-mime="' + mode.mime + '" data-mode="' + mode.mode + '" href="#">' + mode.name + '</a></li>');
		//$modeMenu.append($entry);
	}
};
$(onLoad);
